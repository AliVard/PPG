...
if alg == 'PL':
    suffix += f'RAND_{sessions_cnt}_{sys.argv[5]}'
else:
...
def learn_one_ltroutput(metric, qid, verbose, y_pred, g, dlr, epochs, lr, exposure, grade_levels, samples_cnt, sessions_cnt):
    s, e = dlr[qid:qid+2]
    ...
    return sorted_docs_s
    

def learn_all_ltroutput(metric, y_pred, g, dlr, epochs, lr, exposure, grade_levels, samples_cnt, sessions_cnt):
    sorted_docs = []
    
#     for qid in trange(dlr.shape[0] - 1, leave=False):
    for qid in range(dlr.shape[0] - 1):
        min_b = learn_one_ltroutput(metric, qid, 0, y_pred, g, dlr, epochs, lr, exposure, grade_levels, samples_cnt, sessions_cnt)
...
def learn_one_PL(metric, qid, verbose, y_pred, g, dlr, epochs, lr, exposure, grade_levels, samples_cnt, sessions_cnt):
    ...
    learner = PL.Learner(logits=np.ones_like(y_pred[s:e]), samples_cnt=samples_cnt, 
...
#     with open(f'/ivi/ilps/personal/avardas/_data/PPG/n/{suffix}_{metric}_results.pkl', 'wb') as f:
#         pickle.dump(res, f)
...
#         pickle.dump(res, f)
    ...
    with open(f'/ivi/ilps/personal/avardas/_data/PPG/n/mslr/{suffix}_mslr_{metric}_results.pkl', 'wb') as f:
        pickle.dump(res, f)
